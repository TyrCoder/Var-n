...
This script creates all required tables in the varon database.
...
def init_database():
    ...
    try:
        ...
        for i, create_table_sql in enumerate(CREATE_TABLES, 1):
            try:
                cursor.execute(create_table_sql)
                table_name = create_table_sql.split('`')[1] if '`' in create_table_sql else "Table"
                print(f"✅ Table {i}/{len(CREATE_TABLES)} created/verified: {table_name}")
            except mysql.connector.Error as err:
                if err.errno == errorcode.ER_TABLE_EXISTS_ERROR:
                    print(f"✅ Table {i}/{len(CREATE_TABLES)} already exists")
                else:
                    print(f"❌ Error creating table {i}/{len(CREATE_TABLES)}: {err}")
        ...
        try:
            ...
            """)
            conn.commit()
            print("✅ Sample admin user ensured to exist")
        ...
        if err.errno == errorcode.ER_ACCESS_DENIED_ERROR:
            print(f"❌ Error: Access denied. Check username and password in .env")
...
if __name__ == "__main__":
    print("="*50)
    print("Varon Database Initialization Script")
    print("="*50)
    print()
    ...
    if success:
        ...
        print("\n❌ Database initialization failed. Check your .env file settings.")
